...
void DFRobot_MCP4725::outputTriangle(uint16_t amp, uint16_t freq, uint16_t offset, uint8_t dutyCycle)
{
    uint64_t starttime;
    uint64_t stoptime;
    uint64_t looptime;
    ...
    uint16_t down_num;
    uint16_t maxV;
    maxV=amp*(4096/(float)_refVoltage);
    if(freq > 100){
        num = 16;
    ...
    for (counter = 0; counter < (maxV-(maxV/up_num)-1); counter+=(maxV/up_num))
    {
        ...
        Wire.beginTransmission(_IIC_addr);
        Wire.write(MCP4725_Write_CMD | (_PowerMode << 1));
        Wire.write(enterV / 16);
        Wire.write((enterV % 16) << 4);
        Wire.endTransmission();
        stoptime = micros();
...
void DFRobot_MCP4725::outputSin(uint16_t amp, uint16_t freq, uint16_t offset)
{
    ...
    //    freq = 100;
    //}
    frame = 1000000/(freq*num); // frame = 1000000/(freq*num);
    for(int i=0;i<num;i++){
